(function ($) {
	...
	// Flag body so CSS can safely hide animated elements only when JS is active
  ...
  if ($legacyForm.length) {
	$legacyForm.validator({ focus: false }).on("submit", function (event) {
	  if (!event.isDefaultPrevented()) {
		event.preventDefault();
		submitContactForm($legacyForm);
	  }
	});
  }
  ...
  function submitContactForm($form) {
	$.ajax({
	  type: "POST",
	  url: "form-process.php", // legacy endpoint
	  data: $form.serialize(),
	  success: function (text) {
		if (text === "success") {
		  contactFormSuccess($form);
		} else {
		  submitMSG(false, text);
		}
	  },
	  error: function () {
		submitMSG(false, "Server error occurred.");
	  }
	});
  }
  ...
  function submitMSG(valid, msg) {
	...
	/* Scroll Animations — IntersectionObserver replaces WOW.js
	   Elements are visible by default (SEO-safe); animated class added on scroll */
	(function () {
